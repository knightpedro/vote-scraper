...
class Post2017Extractor:
    summary = "overall-results-summary.csv"
    electorate_party_turnout = "party-votes-and-turnout-by-electorate.csv"
    electorate_candidate_turnout = "candidate-votes-and-turnout-by-electorate.csv"
    electorate_votes_for_registered_parties = "votes-for-registered-parties-by-electorate.csv"
    ...
    def get_voting_place_candidate_filename(self, electorate_number):
        return os.path.join(self.data_path, f"candidate-votes-by-voting-place-{electorate_number}.csv")

    def get_voting_place_party_filename(self, electorate_number):
        return os.path.join(self.data_path, f"party-votes-by-voting-place-{electorate_number}.csv")
    ...
    def flatten_voting_places(self, places):
        flattened = []
        for (place, value) in places.items():
            for (candidate, votes) in value["results"].items():
                flattened.append({
                    "name": place,
                    "suburb": value["suburb"],
    ...
    def get_booth_results(self, electorate_number, party=False):
        ...
        while reader:
            row = next(reader)
            if len(row) < 2 or "Voting Places" in row:
                continue
            if "Overseas Special" in row[1]:
                break
            suburb = row[0]
            booth_name = row[1]
            if booth_name not in voting_places:
                booth = {
                    "electorate": electorate,
                    "suburb": suburb,
                    "results": {candidate: 0 for candidate in candidates}
                }
                voting_places[booth_name] = booth

            for i, votes in enumerate(row[2:-2]):
                candidate = candidates[i]
                voting_places[booth_name]["results"][candidate] += int(votes)
        ...
        return self.flatten_voting_places(voting_places), electorate_results
